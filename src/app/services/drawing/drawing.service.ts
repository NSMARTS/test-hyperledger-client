...
export class DrawingService {
  ...
  start(
    ...
    points: number[],
    tool: { type: any; color: any; width: number }
  ) {
    switch (tool.type) {
      case 'pen':
  ...
  move(
    context: CanvasRenderingContext2D,
    points: string | any[],
    tool: { width: number; color: any; type: any },
    zoomScale: number,
    ...
    context: CanvasRenderingContext2D,
    points: string | any[],
    tool: { width: number; color: any; type: string }
  ) {
    context.lineCap = 'round';
    context.lineJoin = 'round';
    context.lineWidth = tool.width;
  ...
  // Thumbnail에 그리기
  drawThumb(
    data: { points: any; tool: any },
    thumbCanvas: { getContext: (arg0: string) => any },
    thumbScale: any
  ) {
  ...
  async rxPointer(
    data: { points: any[]; tool: { type: string } },
    sourceCanvas: {
